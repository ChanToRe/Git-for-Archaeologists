...
## Git?, Github?
&nbsp; 2000년대 이후 활발한 발굴조사를 통해 수많은 종류의 고고학 자료가 빠른 속도로 누적되고 있다. 한 해 이루어지는 발굴조사가 수 백, 수 천건에 달하고 연구논문 또한 수 백건에 달하기에 개인이 고고학 자료를 수집하고 관리하는 것이 점차 어려워지고 있는 것이 현실이다. 이에 따라 최근에는 자료를 공동으로 수집하고 관리하는 경우가 많아졌고, 나아가 공동연구 사례도 확연히 증가하였다. 이러한 '공동작업'은 현 시점에서 필수적이지만, 단독연구와는 상당한 차이를 보이기 때문에 연구자에게 많은 어려움을 주기도 한다.

&nbsp; 아마 가장 큰 어려움은 자료수집, 원고작성 등에서 발생하는 버전의 충돌일 것이다. 공동연구를 하다보면 한 원고나 데이터베이스를 2명 이상의 연구자가 동시에 수정하는 상황이 발생하는데, 이로 인해 한 가지의 원고나 데이터베이스가 두 개의 버전으로 분화되기도 한다. 이러한 상황에 처한다면 추후 이를 어떻게 병합할지에 대해 고민이 생기기 마련이다. 이때, 가장 편한 방법은 하나의 버전을 폐기하는 것이지만, 이는 최선의 방법이라고 할 수 없다. 이때 최선의 방법은 두 버전을 보고 장점만 추려서 새로운 버전을 만드는 것이다. 하지만 이론적인 개념은 쉽지만 이를 실제로 적용하기는 어려운데, 본 글에서 다룰 Git과 Github를 사용한다면 이를 보다 쉽게 처리할 수 있다.

&nbsp; Git과 Github는 연구결과물을 공유하는 것에도 유용하게 사용될 수 있다. 시간이 지날수록 고고학에서 자연과학적, 통계학적 방법론이 광범위하게 사용되고 있는데, 자연과학계에서 검증 과정을 연구의 필수 과정으로 인식하고 중시하는 반면, 한국고고학계에서는 그러한 검증이 잘 이루어지지 않고 있다. 이러한 양상은 검증에 필요한 최소한의 자료도 공유되지 않는 것에서 비롯된 현상으로 보이는데, 이는 공유할 수 있는 환경이 조성되어있지 않기 때문으로 판단된다. 물론 학회 차원에서 공유할 수 있는 환경을 조성해주는 것이 가장 좋겠지만 당장 해결이 되지 않는다면 본 글에서 다루는 Git과 Github가 유용하게 사용될 수 있다. 

&nbsp; 먼저 Git은 Linus Torvalds에 의해 개발된 **분산 버전 관리 시스템** 이다. 다양한 파일의 변경사항을 기록하여, 추후 필요할 때 이를 확인하거나 되돌릴 수 있다. 이는 여러 사람이 함께 하나의 프로젝트, 연구, 개발을 진행할 때 효율적으로 협업을 할 수 있게 해준다. Git을 사용하면 인터넷이 불가능한 지역에서도 작업이 가능하며, 자연스럽게 백업이 되어 컴퓨터가 분실 혹은 고장나도 복구가 가능하다(Push까지 무사히 끝낸 경우 한정). 무엇보다 가장 큰 장점은 버전을 깔끔하게 정리해준다는 것인데, 이를 통해 복잡한 상황속에서도 깔끔하게 연구, 개발을 진행할 수 있다.

&nbsp; 이어서 **Github**는 Git을 사용하는 프로젝트를 지원하는 웹 호스팅 서비스이다. 즉, **프로젝트의 서버** 역할을 하는데, Git을 웹에서 편히 사용할 수 있게 해주는 도구로 버전 관리, 코드 확인, 버그 정리를 할 수 있으며 블로그, SNS, 포트폴리오 정리의 기능을 해주기도 한다.

*깃허브 가입 및 세팅은 아래의 링크를 참조<br>
...
&nbsp; Git을 사용하는 방법은 크게 두 가지가 있다. 첫 번째, **GUI 프로그램** 으로 우리가 평소에 사용하는 프로그램처럼 되어 있어 사용이 간편하지만 할 수 있는 작업이 적다. 두 번째, **터미널 명령어(CLI)를 활용하는 방법** 은 모든 기능을 사용할 수 있지만 어렵다는 단점이 있다(물론 숙달되면 간단하다).

&nbsp; 본문에서는 **두 번째 방법(터미널 명령어를 활용한 방법)** 을 사용하고자 한다. 명령어를 사용하는 것이 다소 생소하여 어렵게 느껴질 수 있지만, 작업 흐름을 익히면 편하게 사용할 수 있기 때문이다. 본문에는 자주 사용하는 명령어만 기재한 것이고, 실제로는 훨씬 많은 명령어가 존재한다.

&nbsp; 모든 명령어는 대상 파일 혹은 폴더의 상위 디렉토리에서 사용해야하고, 대상 파일 혹은 폴더의 정확한 주소를 기입하여 사용해야한다.
...
Ex) cd..
```
디렉토리를 이동하는 명령어. 'cd ..' 의 경우 현재 디렉토리의 상위 디렉토리로 이동.
...
```
파일 혹은 폴더의 위치를 변경하는 명령어로 흔히 사용하는 ctrl + c, ctrl + v와 동일.
...
Ex) clear
```
터미널 창을 초기화하는 명령어. 명령어를 많이 입력하여 터미널 창이 지저분할 때 사용.
...
Ex) cat script.R
```
파일의 내용을 출력하는 명령어. 단순 출력이기 때문에 편집은 불가능.
...
&nbsp; 아래에서는 Git의 기본적인 용어에 대해 살펴본다. 이러한 용어는 뒤이어 살펴볼 Git의 기본 명령어에서 그대로 사용되는 경우가 많기 때문에 숙지가 필요하다.
...
<img src="https://user-images.githubusercontent.com/64909586/191975497-cd608001-eac7-4637-8801-a8f28e33856c.png">
