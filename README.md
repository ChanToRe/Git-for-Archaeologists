...
## Git?, Github?
&nbsp; 대한민국에서는 2000년대 이후 활발한 발굴조사를 통해 수많은 종류의 고고학 자료가 빠른 속도로 누적되고 있습니다. 한 해 이루어지는 발굴조사가 수 백, 수 천건에 달하고 연구논문 또한 수 백건에 달하기에 개인이 고고학 자료를 수집하고 관리하는 것이 점차 어려워지고 있습니다. 이에 따라 최근에는 자료를 공동으로 수집하고 관리하는 경우가 많아졌습니다. 나아가 공동연구 사례가 이전에 비해 확연히 증가한 것도 쉽게 확인할 수 있습니다. 이러한 '공동작업'은 현 시점에서 필수적이지만 연구과정에서 단독연구와는 상당한 차이를 보이기 때문에 연구자가 많은 어려움을 겪기도 합니다.

&nbsp; 아마 가장 큰 어려움은 자료수집, 원고작성 등에서 발생하는 버전의 충돌일 것입니다. 공동연구를 하다보면 한 원고나 데이터베이스를 2명 이상의 연구자가 동시에 수정하는 상황이 발생합니다. 이로 인해 한 가지의 원고나 데이터베이스가 두 개의 버전으로 분화하게 되는데, 추후 이를 어떻게 병합할지에 대해 고민이 생기기 마련입니다. 물론 가장 편한 방법은 하나의 버전을 폐기하는 것입니다만, 최선의 방법은 역시 두 버전을 보고 장점만 추려서 새로운 버전을 만드는 것 입니다. 하지만 개념은 쉽지만 이를 실제로 적용하기는 어려운데, 이를 간단하게 만들어주는 것이 본 글에서 다룰 Git과 Github입니다.

&nbsp; 시간이 지날수록 고고학에서 자연과학적, 통계학적 방법론이 광범위하게 사용되고 있습니다. 그런데 방법론 차용의 대상이 되는 자연과학계에서는 검증 과정을 연구의 필수적인 과정으로 인식하고 중시하는 반면, 한국고고학계에서는 그러한 검증이 잘 이루어지지 않고 있습니다. 이는 검증에 필요한 최소한의 자료도 공유되지 않는 것에서 비롯된  현상으로 보이는데, 이는 공유할 수 있는 환경이 조성되어있지 않기 때문에 발생하는 것으로 판단됩니다. 그러므로 자료를 공유할 수 있는 환경이 필요한데, 여기서 본 글에서 다루는 Git과 Github가 유용하게 사용될 수 있습니다. 

&nbsp; 이정도면 Git과 Github에 대한 호기심이 생기셨을 것이라고 생각하고 본격적으로 Git과 Github를 살펴보도록 하겠습니다. 먼저 Git은 Linus Torvalds에 의해 개발된 **분산 버전 관리 시스템**입니다. 다양한 파일의 변경사항을 기록하여, 추후 필요할 때 이를 확인하거나 되돌릴 수 있습니다. 이는 여러 사람이 함께 하나의 프로젝트, 연구, 개발을 진행할 때 효율적으로 협업을 할 수 있게 해줍니다. Git을 사용하면 인터넷이 불가능한 지역에서도 작업이 가능하며, 자연스럽게 백업이 되어 컴퓨터가 분실 혹은 고장나도 복구가 가능합니다(Push까지 무사히 끝낸 경우). 무엇보다 가장 큰 장점은 버전을 깔끔하게 정리해준다는 것인데, 이를 통해 다양한 사람들이 함께 협업하는 상황속에서도 깔끔하게 프로젝트, 연구, 개발을 진행할 수 있습니다.
